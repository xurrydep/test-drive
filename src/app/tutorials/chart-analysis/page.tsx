...
import { chartAnalysisModules } from '@/data/modules';
import { chartAnalysisCode } from '@/data/codeSnippets';
import { getProgress, toggleModuleComplete } from '@/utils/progressTracking';

const TUTORIAL_ID = 'chart-analysis';
...
export default function ChartAnalysisPage() {
  ...
  const sidebarModules = chartAnalysisModules.map((m) => ({
    id: m.id,
    title: m.title,
    completed: completed.includes(m.id),
  }));

  const currentMod = chartAnalysisModules[currentModule];
  const codeExample = chartAnalysisCode.find((c) => c.moduleId === currentMod.id);

  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 py-8 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white">📊 Chart Analysis Bot Tutorial</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            Build technical analysis bots with RSI, MACD, and Bollinger Bands.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          <div className="lg:col-span-1">
            <Sidebar
              ...
              tutorialTitle="Chart Analysis Bot"
            />
          </div>

          <div className="lg:col-span-3 space-y-6">
            <ProgressTracker
              totalModules={chartAnalysisModules.length}
              completedModules={completed.length}
              tutorialId={TUTORIAL_ID}
            />

            {chartAnalysisModules.map((mod) => (
              <ModuleStep
                key={mod.id}
            ...
            <div className="flex justify-between pt-4">
              ...
              <button
                onClick={() => setCurrentModule((m) => Math.min(chartAnalysisModules.length - 1, m + 1))}
                disabled={currentModule === chartAnalysisModules.length - 1}
